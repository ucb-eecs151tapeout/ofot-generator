...
def bin(x, width):
    if x < 0: x = (~x) + 1 #RH: Two's complement conversion in case x is negative.
    return ''.join([(x & (1 << i)) and '1' or '0' for i in range(width-1, -1, -1)]) #RH: List comprehension to generate the binary number bit by bit.

def sra(a, b):
    if b == 0:
        return a
    elif a & (1<<31): #RH: Check to see if the MSB is high, indicating negative or not.
        out = (a >> b) | reduce(lambda a,b: a|b, [1 << x for x in range(31, 31-b, -1)]) #RH: Reduce applies the function to the entire iterable until only 1 value remains.
        return out
    else:
        out = a >> b
        return out
...
def sext(a): 
    if not a & (1<<15):
        ...
        return 0xffff0000 | (abs(a)&0xffff) #RH: Sign extends a 16 bit number to 32 bit.

def bwnot(a):
    return reduce(lambda a,b: a|b, [([1,0][a>>x & 1]) << x for x in range(0,32)]) #RH: Bitwise not.
...
functs_rtype = \
{
    "ADD":     ("000", "0", lambda a,b: a+b, lambda a: a, lambda b: b), #RH: This one was the original template.
    ...
    "SLT":     ("010", "0", lambda a,b: (lambda:0, lambda:1)[a<b](), lambda a: a, lambda b: b), #RH: Lambda function to output 1 or 0 based on the comparison.
    "SLTU":    ("011", "0", lambda a,b: (lambda:0, lambda:1)[comp(a,b)](), lambda a: a, lambda b: b),
}
