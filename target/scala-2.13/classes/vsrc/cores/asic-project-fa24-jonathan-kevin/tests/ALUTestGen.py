...
file = open('tests/testvectors.input', 'w')

def gen_vector(op, f, a, b, opcode, funct3, funct7):
    A = a(random.randint(0, 0xffffffff))
    B = b(random.randint(0, 0xffffffff))
    REFout = f(A,B)
    # print(opcode)source /home/ff/eecs151/asic/eecs151.bashrc
    return ''.join([opcode, funct3, funct7, bin(A, 32), bin(B, 32), bin(REFout, 32)][::-1])
#
loops = 25
